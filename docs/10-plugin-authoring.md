...
- Порядок пунктов внутри секции = порядок модулей в `createRegistry([...])`; поле `order` у пункта
  меню **сейчас игнорируется**. `sectionOrder` работает (у `admin` = 900, у `main` задан в crmModule = 10).
...
- Нет фоновых задач, cron, очередей, вебхуков, исполнения действий по расписанию/событию.
...
Нет: динамической загрузки плагинов; фоновых задач/cron/очередей; вебхуков и исходящих интеграций;
